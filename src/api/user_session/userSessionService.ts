...
export class UserSessionService implements IUserSessionService {
  ...
  storeUserSession = async ({
    ...
    try {
      ...
      const existingSession = await this.userSessionRepository.findOne({
        where: { userId, status: UserSessionStatus.ACTIVE, loginType }
      })
      if (existingSession) {
  ...
  getLoginType = async ({ userId }: IGetLoginType) => {
    let code: ResponseCode = ResponseCode.OK
  ...
  getSessionsByType = async ({ userId }: IGetLoginType) => {
    let code: ResponseCode = ResponseCode.OK
