...
//Limit to 1.5 searches/second over 10 seconds
const placesAPILimiterInMemory = new limiter.RateLimiterMemory({
  points: 15,
  duration: 10,
  blockDuration: 10
})

export const placesAPIRateLimiter = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    await placesAPILimiterInMemory.consume(req.ip!)
    next()
  } catch (error) {
    return next({ code: ResponseCode.TOO_MANY_REQUESTS })
  }
...
const languagesAPILimiterInMemory = new limiter.RateLimiterMemory({
  points: 15,
  duration: 10,
  blockDuration: 10
})

export const languagesAPIRateLimiter = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    await languagesAPILimiterInMemory.consume(req.ip!)
    next()
  } catch (error) {
    return next({ code: ResponseCode.TOO_MANY_REQUESTS })
  }
